...
if __name__ == "__main__":
    ...
    dir_cur_name = './model/' + args.dataset[10:] + '_' + cur_Method + '/'
    # save txt for train and valid
    file_train_loss = open(dir_cur_name + cur_Method + "_Loss_train.txt", 'w')
    file_valid_loss = open(dir_cur_name + cur_Method + "_Loss_valid.txt", 'w')

    if args.seed != 0:
        torch.manual_seed(args.seed)

    trainTotal, trainList, trainDict = loadTriple(args.dataset, 'train2id.txt')
    validTotal, validList, validDict = loadTriple(args.dataset, 'valid2id.txt')
    tripleTotal, tripleList, tripleDict = loadTriple(args.dataset, 'triple2id.txt')
    config = Config()
    ...
    config.entity_total = getAnythingTotal(config.dataset, 'entity2id.txt')
    config.relation_total = getAnythingTotal(config.dataset, 'relation2id.txt')
    ...
    for epoch in range(config.train_times):
        ...
        for batchList in trainBatchList:
            ...
            normalize_norm_emb = F.normalize(model.norm_embeddings.weight.data[longTensor(batch_relation_list)], p=2,
                                             dim=1)
        ...
        if epoch % 10 == 0:
            ...
            # # save embedding
            # ent_embeddings_print = model.ent_embeddings.weight.data.cpu().numpy()
            # rel_embeddings_print = model.rel_embeddings.weight.data.cpu().numpy()
            # norm_embeddings_print = model.norm_embeddings.weight.data.cpu().numpy()
            #
            # file_embed_t = open(dir_cur_name + "embedding_ent_epoch_" + ("%4.4d" % epoch) + ".txt", 'w')
            # file_embed_t.write(str(ent_embeddings_print.tolist()))
            # file_embed_t.close()
            #
            # file_embed_t = open(dir_cur_name + "embedding_rel_epoch_" + ("%4.4d" % epoch) + ".txt", 'w')
            # file_embed_t.write(str(rel_embeddings_print.tolist()))
            # file_embed_t.close()
            #
            # file_embed_t = open(dir_cur_name + "embedding_weight_epoch_" + ("%4.4d" % epoch) + ".txt", 'w')
            # file_embed_t.write(str(norm_embeddings_print.tolist()))
            # file_embed_t.close()
            ...
            # print("Valid batch loss: %d %f" % (epoch, losses.data.item()))
        ...
        if config.early_stopping_round > 0:
            if epoch == 0:
                ...
                hit10, best_meanrank = evaluation_transH(validList, tripleDict, ent_embeddings, rel_embeddings,
                                                         norm_embeddings, L1_flag, filter, k=config.batch_size,
    ...
    norm_embeddings = model.norm_embeddings.weight.data.cpu().numpy()
    L1_flag = model.L1_flag
    filter = model.filter
    ...
    file_embed_t = open(dir_cur_name + "embedding_weight_final.txt", 'w')
    file_embed_t.write(str(norm_embeddings.tolist()))
    file_embed_t.close()

    # my addition
    with open(dir_cur_name + cur_Method + '_%s_%s_best.pkl' % (config.dataset[26:], str(config.embedding_size)), 'wb') as f_t:
    ...
    # filter = model.filter
    #
    # hit10Test, meanrankTest = evaluation_transH(testList, tripleDict, ent_embeddings, rel_embeddings, norm_embeddings,
    #                                             L1_flag, filter, head=0)
    #
    # hit10OneToOneHead, meanrankOneToOneHead = evaluation_transH(oneToOneList, tripleDict, ent_embeddings,
    #                                                             rel_embeddings, norm_embeddings, L1_flag, filter,
    #                                                             head=1)
    # hit10OneToManyHead, meanrankOneToManyHead = evaluation_transH(oneToManyList, tripleDict, ent_embeddings,
    #                                                               rel_embeddings, norm_embeddings, L1_flag, filter,
    #                                                               head=1)
    # hit10ManyToOneHead, meanrankManyToOneHead = evaluation_transH(manyToOneList, tripleDict, ent_embeddings,
    #                                                               rel_embeddings, norm_embeddings, L1_flag, filter,
    #                                                               head=1)
    # hit10ManyToManyHead, meanrankManyToManyHead = evaluation_transH(manyToManyList, tripleDict, ent_embeddings,
    #                                                                 rel_embeddings, norm_embeddings, L1_flag, filter,
    #                                                                 head=1)
    #
    # hit10OneToOneTail, meanrankOneToOneTail = evaluation_transH(oneToOneList, tripleDict, ent_embeddings,
    #                                                             rel_embeddings, norm_embeddings, L1_flag, filter,
    #                                                             head=2)
    # hit10OneToManyTail, meanrankOneToManyTail = evaluation_transH(oneToManyList, tripleDict, ent_embeddings,
    #                                                               rel_embeddings, norm_embeddings, L1_flag, filter,
    #                                                               head=2)
    # hit10ManyToOneTail, meanrankManyToOneTail = evaluation_transH(manyToOneList, tripleDict, ent_embeddings,
    #                                                               rel_embeddings, norm_embeddings, L1_flag, filter,
    #                                                               head=2)
    # hit10ManyToManyTail, meanrankManyToManyTail = evaluation_transH(manyToManyList, tripleDict, ent_embeddings,
    #                                                                 rel_embeddings, norm_embeddings, L1_flag, filter,
    #                                                                 head=2)
    #
    # writeList = [filename,
    ...
    # with open(os.path.join('./result/', args.dataset + '.txt').replace('\\', '/'), 'a') as fw:
