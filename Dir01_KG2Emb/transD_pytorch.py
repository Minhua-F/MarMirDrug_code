...
#   evaluation_list = []
    ...
    file_evaluate_value = open(dir_cur_name + cur_Method + "_hit10_meanrank.txt", 'w')
    ...
    filename = '_'.join(
        ['l1', str(args.learning_rate1),
         ...
         'lo', str(args.loss_type), ]) + '_TransD.ckpt'
        ...
        if config.early_stopping_round > 0:
            if epoch == 0:
                ...
                hit10, meanrank = evaluation_transD(validList, tripleDict, ent_embeddings, rel_embeddings,
                                                    ent_proj_embeddings, rel_proj_embeddings, L1_flag, filter,
                                                    k=config.batch_size, num_processes=args.num_processes)
                worst_hit10 = hit10
                best_meanrank = meanrank

                file_evaluate_value.write("%d\t%f\t%f\n" % (epoch, hit10, meanrank))
