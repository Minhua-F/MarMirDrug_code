...
if __name__ == "__main__":
    ...
    for epoch in range(config.train_times):
        ...
        if epoch % 10 == 0:
            ...
            # # Cannot find "proj_embeddings"!
            proj_embeddings_print = model.proj_embeddings.weight.data.cpu().numpy()
            print(proj_embeddings_print)
        ...
        if config.early_stopping_round > 0:
            if epoch == 0:
                ...
                hit10, meanrank = evaluation_transR(validList, tripleDict, ent_embeddings, rel_embeddings,
                                                    proj_embeddings, L1_flag, filter, k=config.batch_size,
    ...
    proj_embeddings = model.proj_embeddings.weight.data.cpu().numpy()
    L1_flag = model.L1_flag
    filter = model.filter
    ...
    file_embed_t = open(dir_cur_name + "embedding_proj_final.txt", 'w')
    file_embed_t.write(str(proj_embeddings.tolist()))
    file_embed_t.close()

    # # my addition
    with open(dir_cur_name + cur_Method + '_%s_%s_best.pkl' % (config.dataset[26:], str(config.ent_embedding_size)), 'wb') as f_t:
    ...
    # filter = model.filter
    #
    # hit10Test, meanrankTest = evaluation_transR(testList, tripleDict, ent_embeddings, rel_embeddings, proj_embeddings,
    #                                             L1_flag, filter, head=0)
    #
    # hit10OneToOneHead, meanrankOneToOneHead = evaluation_transR(oneToOneList, tripleDict, ent_embeddings,
    #                                                             rel_embeddings, proj_embeddings, L1_flag, filter,
    #                                                             head=1)
    # hit10OneToManyHead, meanrankOneToManyHead = evaluation_transR(oneToManyList, tripleDict, ent_embeddings,
    #                                                               rel_embeddings, proj_embeddings, L1_flag, filter,
    #                                                               head=1)
    # hit10ManyToOneHead, meanrankManyToOneHead = evaluation_transR(manyToOneList, tripleDict, ent_embeddings,
    #                                                               rel_embeddings, proj_embeddings, L1_flag, filter,
    #                                                               head=1)
    # hit10ManyToManyHead, meanrankManyToManyHead = evaluation_transR(manyToManyList, tripleDict, ent_embeddings,
    #                                                                 rel_embeddings, proj_embeddings, L1_flag, filter,
    #                                                                 head=1)
    #
    # hit10OneToOneTail, meanrankOneToOneTail = evaluation_transR(oneToOneList, tripleDict, ent_embeddings,
    #                                                             rel_embeddings, proj_embeddings, L1_flag, filter,
    #                                                             head=2)
    # hit10OneToManyTail, meanrankOneToManyTail = evaluation_transR(oneToManyList, tripleDict, ent_embeddings,
    #                                                               rel_embeddings, proj_embeddings, L1_flag, filter,
    #                                                               head=2)
    # hit10ManyToOneTail, meanrankManyToOneTail = evaluation_transR(manyToOneList, tripleDict, ent_embeddings,
    #                                                               rel_embeddings, proj_embeddings, L1_flag, filter,
    #                                                               head=2)
    # hit10ManyToManyTail, meanrankManyToManyTail = evaluation_transR(manyToManyList, tripleDict, ent_embeddings,
    #                                                                 rel_embeddings, proj_embeddings, L1_flag, filter,
    #                                                                 head=2)
    #
    # writeList = [filename,
