...
class TransHModel(nn.Module):
    ...
    def forward(self, pos_h, pos_t, pos_r, neg_h, neg_t, neg_r):
        ...
        pos_h_e = projection_transH_pytorch(pos_h_e, pos_norm)  # w^T h w
        pos_t_e = projection_transH_pytorch(pos_t_e, pos_norm)  # w^T t w
        neg_h_e = projection_transH_pytorch(neg_h_e, neg_norm)  # w^T h w
        neg_t_e = projection_transH_pytorch(neg_t_e, neg_norm)  # w^T t w
...
# 使用预训练嵌入的 TransR。
# 预训练嵌入使用 TransE 进行训练，存储在 './transE_%s_%s_best.pkl'，
# 以 '%s' 数据集名称为首，
# 第二个 '%s' 嵌入大小。
# 使用单位矩阵初始化投影矩阵。
...
class TransRPretrainModel(nn.Module):
    def __init__(self, config):
        ...
        with open('./model/' + config.dataset[10:] + '_transE_pytorch/' + 'transE_pytorch_%s_%s_best.pkl' % (config.dataset[26:], str(config.ent_embedding_size)), 'rb+') as fr:
        ...
        #     ent_weight_cpu = ent_weight.cpu()
        #     np.savetxt(log_file, ent_weight_cpu.detach().numpy())
        # print('haha 2')
        ...
        nn.init.eye_(proj_weight)
        proj_weight = proj_weight.view(-1).expand(self.relation_total, -1).clone()
...
# 和实体和关系的嵌入大小相同。
# 可以看作是TransH的一个特例，
# （参见 “Knowledge Graph Embedding via Dynamic Mapping Matrix” 论文）
# 预训练嵌入使用 TransE 进行训练，存储在 './transE_%s_%s_best.pkl'，
# 以 '%s' 数据集名称为首，
# 第二个 '%s' 嵌入大小。
# 初始化零矩阵的投影矩阵。
class TransDPretrainModelSameSize(nn.Module):
    def __init__(self, config):
        ...
        with open('./model/' + config.dataset[10:] + '_transE_pytorch/' + 'transE_pytorch_%s_%s_best.pkl' % (config.dataset[26:], str(config.embedding_size)), 'rb+') as fr:
            T_out = pickle.load(fr)
            ent_embeddings_list = T_out.ent_embeddings
            rel_embeddings_list = T_out.rel_embeddings
            input_ent = torch.cuda.LongTensor(range(ent_embeddings_list.num_embeddings)) # change embedding to torch
