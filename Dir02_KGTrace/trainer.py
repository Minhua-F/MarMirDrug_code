...
class Trainer(object):
    def __init__(self, params):
        ...
        root_dir = './'
        data_dir = 'datasets/data_preprocessed/FB15K-237/'
        data_dir1 = 'KG2ID_iRefIndex/'
        data_dir2 = 'D_DrugCV_MultiClass_000_CV_05_01/'
        data_dir2 = 'D_DrugCV_MultiClass_000_CV_05_01/'
        data_dir3 = 'D_DrugCV_MultiClass_000_CV_05_01_transE/'
        clusterid_dir = root_dir + data_dir + data_dir1 + data_dir2 + data_dir3
        f1 = open(os.path.join(clusterid_dir, 'entity2clusterid.txt'))
    ...
    def calc_cum_discounted_reward_credit(self, approx_credits, entity_rewards, cluster_rewards):
        ...
        for t in reversed(range(1, self.path_length)):
            running_add = self.gamma * running_add + cum_disc_reward[:,
                                                     t] + cluster_rewards  # approx_credits[t].to(self.device) * cluster_rewards
    ...
    def train(self):
        ...
        for entity_episode, cluster_episode in self.train_environment.get_episodes(self.batch_counter):
            ...
            prev_relation = self.e_agent.dummy_start_label.to(
            ...
            for i in range(self.path_length):
                ...
                entity_state, whether_e_agent_follows_c_agent = entity_episode(idx, prev_cluster.cpu(),
                                                                               i)  ## important !! switch to next state with new entity and new relation
                ...
                entity_episode.get_stepwise_approximated_reward(current_entities_t, current_clusters_t,
                                                                prev_entities)  ## estimate the reward by taking each step
                prev_entities = current_entities_t.clone()
    ...
    def test(self, beam=False, print_paths=False, save_model=True, auc=False):
        ...
        with torch.no_grad():
            ...
            for entity_episode, cluster_episode in tqdm(self.test_environment.get_episodes(0)):
                ...
                # #test save tensor
                # torch.save(query_relation, self.output_dir + '/' + str(batch_counter) + 'query_relation.pt')
                ...
                for i in range(self.path_length):
                    ...
                    if beam:
                        ...
                        y = torch.div(idx, self.max_num_actions, rounding_mode='trunc')
                    ...
                    cluster_state = cluster_episode.next_action(
                        c_idx)  ## important !! switch to next state with new cluster
                ...
                ##############################
                # 加一个向量“pos_of_batch”: 长为temp_batch_size的一个数值向量，每个位置的元素，记录这个b对应的最终answer_pos
                ##############################
                pos_of_batch = np.empty([temp_batch_size, 1])  ##################
                ...
                ######## 存变量为pkl文件 ##########
                with open(self.output_dir + '/pos_of_batch.pkl', 'ab') as fw:  ##################
                    pickle.dump(pos_of_batch, fw)  ##################
                with open(self.output_dir + '/log_probs.pkl', 'ab') as fw:  ##################
                    pickle.dump(self.log_probs, fw)  ##################
                with open(self.output_dir + '/sorted_indx.pkl', 'ab') as fw:  ##################
                    pickle.dump(sorted_indx, fw)  ##################
...
def read_pretrained_embeddings(options: object) -> object:

    entity2vec = np.loadtxt(options['bern_txt_dir'] + '/entity2vec.bern')
    relation2vec = np.loadtxt(options['bern_txt_dir'] + '/relation2vec.bern')
    print(entity2vec.shape)
    # assert entity2vec.shape[1] == 2 * options['embedding_size']

    f1 = open(options['txt_dir'] + '/entity2id.txt')
    f2 = open(options['txt_dir'] + '/relation2id.txt')
    next(f1)
    next(f2)
    ...
    for line in relation2id:
        ...
        relation = str_list[0]
        rid = str_list[1]
        relation2emb[relation] = relation2vec[int(rid)]
    # relation2emb[line.split()[0]] = relation2vec[int(line.split()[1])]
    for line in entity2id:
        str_list = line.strip("\n").split("\t")
        entity = str_list[0]
        eid = str_list[1]
        entity2emb[entity] = entity2vec[int(eid)]
...
if __name__ == '__main__':
    ...
    # read command line options读取命令行选项
    options = read_options()
    options = read_pretrained_embeddings(options)  # 读预训练的嵌入向量
    ...
    # read the vocab files, it will be used by many classes hence global scope读全局变量，json.load将json转化为python数据
    ...
    #
    # with open ('./test/relation_embeddings.txt', 'wb') as f:
    # 	pickle.dump(relation_embeddings, f)
    # with open ('./test/entity_embeddings.txt', 'wb') as f:
    ...
    trainer.test(beam=True, print_paths=True, save_model=True)
